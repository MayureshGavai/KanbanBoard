...
export default function AddTask({ setData }) {
  let [isOpen, setIsOpen] = useState(false);
  const { addTask, taskData } = useContext(TaskContext);
    ...
    <>
      <div className="flex items-center justify-center font-Archivo">
        <button
          type="button"
          onClick={openModal}
          className="rounded-md bg-black px-4 py-2 text-xs md:text-sm  font-medium text-white hover:bg-black/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/75"
        >
          Add new task
        </button>
      </div>
                  ...
                  >
                    <div className="flex justify-between items-center my-2">
                      ...
                      <div className="w-2/3">
                        <input
                          type="text"
                          {...register("taskName", { required: true, maxLength: 24 })}
